...
describe('actions', () => {
	...
	describe('pinItem / unpinItem', () => {
		it('can be used to pin and unpin multiple items using successive calls', () => {
			...
			registry.dispatch(interfaceStore).pinItem('my-plugin', 'ui-item-1');

			registry.dispatch(interfaceStore).pinItem('my-plugin', 'ui-item-2');
	...
	describe('toggleFeature', () => {
		it('changes a value that was true to false', () => {
			...
			registry.dispatch(interfaceStore).toggleFeature('test', 'feature1');
			expect(
				registry
					.select(interfaceStore)
					.isFeatureActive('test', 'feature1')
